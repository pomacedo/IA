...
import goals.agents.*;
import goals.objects.Bush;
import goals.objects.House;
import goals.objects.SimObject;
import goals.objects.Tree;
...
public class SimEnvironment implements Steppable{
	...
	public SimEnvironment(SimState state, int width, int height, int nAgents,SimObject interest){
		...
		this.broker = new BrokerAgent(this.mapper);
		                          
		this.setup(state,interest);
	 ...
	 */
	private void setup(SimState state,SimObject interest){
		
		addExplorersRandomly(state, interest);
	...
	private void addExplorersRandomly(SimState state,SimObject interest) {
		for(int i= 0; i < explorers.capacity(); i++){
			...
			addExplorer(state, loc,interest);
		}
//		Int2D loc = new Int2D(0, world.getHeight());
//		addExplorer(state, loc);
	}
	
	private void addExplorersCornersCenter(SimState state,SimObject interest) {
		...
		for (Int2D l : locs)
			addExplorer(state, l,interest);
		
	}
	
	private void addExplorer(SimState state, Int2D loc,SimObject interest) {
		ExplorerAgent explorer = new ExplorerAgent(loc,interest);
	...
	private void printStats() {
		...
		System.err.println("% WATER FOUND  "+nWater +" / 1000"+"%"+"at "+GoalStep);
