...
public class BrokerAgent {
	...
	private final int NUM_REL_OBJ = 3;
	
	public BrokerAgent(MapperAgent mapper) {
...
// 	}
	...
	public void addRelativePOI(Bag points, double interestMeasure)
	{
				
		int num=0;
		List<Integer> forbiden = new ArrayList<Integer>();
		
		while(num<NUM_REL_OBJ)
		{
			...
			do
			{
				pos=rand.nextInt(points.size()-num);
			}
			while(forbiden.contains(pos));
			forbiden.add(pos);
			SimObject o = (SimObject) points.get(pos);
			
			
			PointOfInterest chk = checkPOI(o.loc);
			if(chk==null)
			{
				if(!checkIfRemoved(o.loc) && !mapper.isIdentified(o.loc))
				{
					pointsOfInterest.add(new PointOfInterest(o.loc, interestMeasure));
					
				}
			}
			else
			{
				pointsOfInterest.get(pointsOfInterest.indexOf(chk)).interestMeasure*=1.4;
...
//	public void decayPoints(Bag points)
//	{
//		int num;
//		List<Integer> forbiden = new ArrayList<Integer>();
//		for(num=0;num<points.size();num++)
//		{
//			SimObject o = (SimObject)points.get(num);
//			PointOfInterest chk = checkPOI(o.loc);
//			if(chk!=null)
//			{
//				pointsOfInterest.get(pointsOfInterest.indexOf(chk)).interestMeasure*=0.8;
//			}
//		}
