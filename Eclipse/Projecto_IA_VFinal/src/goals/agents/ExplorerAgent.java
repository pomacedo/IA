...
public class ExplorerAgent implements sim.portrayal.Oriented2D {
	...
	public goals.env.SimEnvironment env;
	...
	private SimObject myInterest;
	
	public ExplorerAgent(Int2D loc, SimObject interest) {
		this.loc = loc;
		this.orientation = 0;
		this.target = null;
		...
		if (identifyClock == 0) {
			...
			for (int i = 1; i < visible.size(); i++) {
				...
				if (!mapper.isIdentified(obj.loc)) {
					Hashtable<Class, Double> probs = getProbabilityDist(obj,getMyInterest());
...
//							+ obj.loc.y + "). INTEREST: " + interest);
					...
					if (interest < INTEREST_THRESHOLD) {
						...
						mapper.identify(obj, highest,myInterest);
...
//				+ unknownInterest);
	...
	private Hashtable<Class, Double> getProbabilityDist(SimObject obj,SimObject interestObj) {
		...
		for (Prototype prot : prototypes) {
			// TODO: Stuff here
			//System.out.println("SIZE PROT: "+prototypes.size());
			Int2D loc = obj.loc;
			
			Bag neighboors = env.getVisibleObejcts(loc.x, loc.y, OBJ_RELATION_RADIOUS);
			int numOcurr=prot.nOccurrs;
...
//			}
			...
			// Saturation
			corr = Utils.saturate(corr,numOcurr);
		...
		//System.out.println("TOTAL: "+corrSum);
